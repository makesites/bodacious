...
var Main = function Main(name, options){
	...
	this.queue = Promise.promisifyAll( new Queue( this.store, options ) ); // support multiple queues?
...
Main.prototype = {
	...
	run: function(){
		...
		//while(i--) {
			promises.push(this.queue.load().then(this.queue.execute.bind(this)))
		//}
