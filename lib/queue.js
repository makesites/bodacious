...
Queue.prototype.process = function( job ){
	...
	// convert handler to a promise
	var handler = Promise.promisify(this.handler.bind(this));
...
Queue.prototype.add = function( job ){
	...
	// get options?
	// add it to the store
	return this.store.createAsync( data );
};
...
Queue.prototype.load = function(){
	...
	return this.store.listAsync( prefix ).then(function( keys ){
		// loop through keys
		var tasks = keys.map(function(key) {
			// get data details
			return self.store.readAsync( key ).then(function( data ){
				// create a job object for each data item
				var job = new Job( data );
				job.on('remove', self.remove.bind(self) );
				//
				return self.process.bind( self, job );
			});
		});
