...
Queue.prototype.process = function( job ){
	...
	return handler( job.toJSON() ).then(handleCompleted, handleFailed).finally(finishProcessing);
};
...
Queue.prototype.load = function( callback ){
	...
	var actions = [
		...
		function( done ){
			async.each( keys, function( key, next ){
				...
				self.store.read( key, function(err, data){
					...
					job.on('remove', self.remove.bind(self) );
					jobs.push( job );
					return
					next();
				});
		...
		// process jobs
		function( done ){
			var method = ( self.options.serial ) ? 'series': 'each';
			async[method]( jobs, function( job, next ){
				// executed
				self.process.bind( self, job ); // callback
			}, function(err, results){
				// error control
				// continue..
				done();
...
/*
	...
	if( options.store == 'redis' ){
		this.client.select(redisDB).then(function(){
			return _this.bclient.select(redisDB);
		}).then(function(){
			return _this.eclient.select(redisDB);
		}).then(function(){
			return _this.eclient.subscribe(_this.toKey('delayed'));
		}).then(function(){
			return _this.eclient.subscribe(_this.toKey('paused'));
		}).then(function(){
			_this.emit('ready');
		});
	} else {
...
Queue.prototype.add = function(data, opts){
	...
	//
	return _this.client.incr(this.toKey('id')).then(function(jobId){
...
Queue.prototype.count = function(){
 ...
 Returns a function that is resolved after the operation has been completed.
...
// This function updates the delay timer, which is a timer that timeout at the next known delayed job.
...
// Process jobs that have been added to the active list but are not being processed properly.
Queue.prototype.processStalledJobs = function(){
	var _this = this;

	return this.client.lrange(this.toKey('active'), 0, -1).then(function(active){
	return active.map(function(jobId){
...
Queue.prototype.processStalledJob = function(job){
	...
	return job.takeLock(_this.token).then(function(lock){
		...
		return _this.client.sismember(key, job.jobId).then(function(isMember){
			if(!isMember){
...
Queue.prototype.processJob = function(job){
	...
	var runHandler = this.handler.bind(this);
	...
	lockRenewer();
	var jobs = runHandler(job);

	if(timeoutMs){
	jobs = jobs.timeout(timeoutMs);
	}

	return jobs.then(handleCompleted, handleFailed).finally(finishProcessing);
}

// Returns a function that resolves to the next job in queue.
...
Queue.prototype.moveJob = function(src, dst, opts) {
	...
	return this.bclient.brpoplpush(this.toKey(src), this.toKey(dst), 0);
	}
}
