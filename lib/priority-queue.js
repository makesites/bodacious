/* THIS FEATURE IS DISABLED IN FAVOR OF CRON-LIKE OPTIONS */
 ...
 */
var PriorityQueue = module.exports = function(name, options) {
	...
	for (var key in PriorityQueue.priorities) {
		var queue = Queue(PriorityQueue.getQueueName(name, key), options);
 ...
 * In order to avoid query too much, and to reduce load, we wait a certain time if we loop all queue
